...
router.post("/load", async (req, res) => {
    var terms = req.body.loadsearchbox;
    ...
    client.get('statuses/user_timeline', {screen_name: terms, count: 200, include_rts: false}, async (error, tweets, response) => {
        if(!error){
            ...
            try{
                ...
                res.render("pages/deepfake", {title: "Deepfakes..", shouldshow: false, err:true, errmsg: "There was a problem loading those tweets into the database."});
                return;
            }
            var shown = [];
...
router.post("/generate", async (req, res) => {
    var terms = req.body.gensearchbox;
    if(!terms || typeof terms != 'string' || terms.length === 0){
        res.redirect('/deepfake');
        return;
    }   
...
router.get("/tweet", async(req, res) =>{
    res.render("pages/deepfake", {title: "Tweet posted successfully", shouldshow: false, tweeted:true});
});
router.post("/tweet", async(req, res) =>{
    var tweet = req.body.tweet;
    var name = req.body.screenname;
    client.post("statuses/update", {status: "(generated from some of @" + name +"'s tweets): " + tweet}, (err, data, response) =>{
